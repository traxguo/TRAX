...
const NotificationPanel = ({onClose}:{onClose:()=>void}) => {
  ...
  return (
    <div style={{position:'absolute',inset:0,zIndex:60,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(6px)',display:'flex',flexDirection:'column',justifyContent:'flex-end'}}>
      <div style={{background:'#0C0C0C',borderRadius:'28px 28px 0 0',borderTop:'1px solid rgba(255,255,255,0.1)',padding:'20px 20px 36px',display:'flex',flexDirection:'column',gap:'14px',maxHeight:'82%',overflow:'hidden',animation:'slideUp 0.3s ease'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <button onClick={onClose} style={{width:'30px',height:'30px',borderRadius:'50%',background:'rgba(255,255,255,0.08)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
          ...
          {(['expiring','expired'] as const).map(type=>(
            <div key={type} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'14px',padding:'12px'}}>
          ...
          {urgents.length===0
            ...
            : <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                {urgents.map(m=>{
                  ...
                  return (
                    <div key={m.id} style={{display:'flex',alignItems:'center',gap:'10px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'14px',padding:'10px 12px'}}>
...
const EditMemberModal = ({member,onClose,onSave}:{member:Member;onClose:()=>void;onSave:(m:Member)=>void}) => {
  ...
  const inp:React.CSSProperties={width:'100%',padding:'13px 14px',borderRadius:'13px',fontSize:'14px',color:'white',outline:'none',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',boxSizing:'border-box'};
...
const Header=({onAddMember,onBell,urgentCount}:{onAddMember?:()=>void;onBell:()=>void;urgentCount:number})=>{
  ...
  const hdr:React.CSSProperties={
    paddingTop: 'calc(env(safe-area-inset-top, 20px) + 24px)', // Logo saatin altına indi
    paddingBottom: '14px', paddingLeft: '20px', paddingRight: '20px',
    display:'flex',alignItems:'center',justifyContent:'space-between',
    position:'sticky',top:0,zIndex:10,background:'rgba(12,12,12,0.92)', // Hafif gri, ayrışıyor
...
const BottomNav=()=>{
  ...
  return(
    <div style={{
      flexShrink:0,
      background:'linear-gradient(to top, rgba(0,0,0,0.95) 50%, transparent)', // Siyahlık daha az yer kaplıyor
      display:'flex',alignItems:'flex-end',justifyContent:'center',
      paddingBottom:'calc(env(safe-area-inset-bottom, 16px) + 16px)', // Safe area düzeltmesi
      paddingTop:'16px', // Çemberi aşağı indirdik
      zIndex:20
    }}>
      <div style={{background:'rgba(18,18,18,0.95)',backdropFilter:'blur(24px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'999px',height:'56px',display:'flex',alignItems:'center',justifyContent:'center',gap:'48px',padding:'0 40px',boxShadow:'0 16px 40px rgba(0,0,0,0.8)'}}>
...
const Layout=()=>{
  ...
  return(
    <StoreContext.Provider value={{members,addMember,updateMember,deleteMember}}>
      {/* 100dvh hatasını height: 100% yaparak düzelttik */}
      <div style={{width:'100%',height:'100%',background:'#000000',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        
        {/* PARLAMA EFEKTİ VE YUMUŞATILMIŞ KART BORDERLARI */}
        <style dangerouslySetInnerHTML={{__html:`
          @keyframes spinGlow { 100% { transform: rotate(360deg); } }
          .glow-wrap {
            position: relative; border-radius: 18px; padding: 1.5px; overflow: hidden;
            background: rgba(255,255,255,0.04); z-index: 1; isolation: isolate; cursor: pointer;
            box-shadow: 0 4px 20px rgba(0,0,0,0.5);
          }
          .glow-wrap::before {
            content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%;
            background: conic-gradient(from 0deg, transparent 40%, var(--glow-color) 80%, transparent 100%);
            filter: blur(12px); /* Düz çizgiyi yumuşak, dönen bir ışığa (flare) çevirir */
            animation: spinGlow 4s linear infinite; z-index: -1;
            opacity: 0.65;
          }
          .glow-inner {
            background: #0C0C0C; border-radius: 17px; width: 100%; height: 100%;
            position: relative; z-index: 1; padding: 13px 14px;
            display: flex; align-items: center; gap: 12px;
            border: 1px solid rgba(255,255,255,0.06); /* Arkaplana blend olmasını engeller */
          }
          .mini-glow-inner {
            background: #0C0C0C; border-radius: 13px; width: 100%; height: 100%;
...
const DashboardScreen=()=>{
  ...
  const card:React.CSSProperties={background:'#0C0C0C',borderRadius:'20px',border:'1px solid rgba(255,255,255,0.08)',padding:'20px',position:'relative',overflow:'hidden'};
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'12px 16px 8px'}}>
        <div style={card}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'12px'}}>
            <div>
              <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}}><Activity style={{width:'13px',height:'13px',color:AL}}/><span style={{fontSize:'11px',fontWeight:600,color:'rgba(255,255,255,0.5)',letterSpacing:'0.1em',textTransform:'uppercase'}}>Haftalık Akış</span></div>
          ...
          <div style={{height:'100px',marginLeft:'-10px',marginRight:'-10px',marginTop:'10px'}}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueData} margin={{top:10,right:10,left:10,bottom:0}}>
                ...
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{fontSize:10, fill:'rgba(255,255,255,0.4)', fontWeight:600}} dy={10} />
                <Tooltip contentStyle={{background:'#0C0C0C', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'12px', fontSize:'12px', fontWeight:700, color:'#fff'}} itemStyle={{color:AL}} cursor={false} />
        ...
        {pending>0&&<div style={{borderRadius:'18px',padding:'14px 16px',display:'flex',alignItems:'center',gap:'12px',background:'rgba(217,119,6,0.05)',border:'1px solid rgba(217,119,6,0.15)'}}>
          ...
          <div style={{flex:1}}><p style={{fontSize:'10px',fontWeight:700,color:'rgba(245,158,11,0.6)',letterSpacing:'0.1em',textTransform:'uppercase'}}>Bekleyen Tahsilat</p><p style={{fontSize:'16px',fontWeight:900,color:AL,lineHeight:1.3}}>₺{pending.toLocaleString('tr-TR')}</p></div>
          <Link to="/app/members" style={{fontSize:'11px',fontWeight:700,padding:'6px 12px',borderRadius:'999px',textDecoration:'none',color:AL,background:'rgba(217,119,6,0.08)',border:'1px solid rgba(217,119,6,0.2)'}}>Görüntüle</Link>
        </div>}

        {urgent.length>0&&<div style={{background:'#0C0C0C',borderRadius:'20px',border:'1px solid rgba(255,255,255,0.08)',overflow:'hidden'}}>
              ...
              <div key={m.id} onClick={()=>navigate(`/app/members/${m.id}`)} className="glow-wrap" style={{'--glow-color': glowColor, borderRadius:'14px'} as any}>
                <div className="mini-glow-inner">
                  <div style={{position:'relative',flexShrink:0}}>
                    ...
                    <span style={{position:'absolute',bottom:'-2px',right:'-2px',width:'12px',height:'12px',borderRadius:'50%',border:'2.5px solid #0C0C0C',background:glowColor}}/>
                  </div>
                  <div style={{flex:1,minWidth:0}}><p style={{fontSize:'13px',fontWeight:600,color:'rgba(255,255,255,0.9)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.name}</p><p style={{fontSize:'11px',color:'rgba(255,255,255,0.4)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.package}</p></div>
                  <span style={{fontSize:'9px', fontWeight:700, padding:'3px 8px', borderRadius:'6px', background: `rgba(255,255,255, 0.05)`, color: glowColor, border: `1px solid rgba(255,255,255, 0.1)`, flexShrink:0}}>{getStatusLabel(m.daysRemaining)}</span>
                </div>
              </div>
            )})}
          </div>
  ...
  return(
    <PageWrapper>
      <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'12px 16px'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'8px', marginTop:'8px', paddingBottom:'20px'}}>
          ...
          {filtered.map((m,i)=>{
            ...
            <div key={m.id} onClick={()=>navigate(`/app/members/${m.id}`)} className="glow-wrap" style={{'--glow-color': hexColor, opacity:0, animation:`fadeUp 0.3s ease ${i*0.05}s forwards`} as any}>
              <div className="glow-inner">
                ...
                <span style={{fontSize:'10px', fontWeight:700, padding:'4px 10px', borderRadius:'8px', background: `rgba(255,255,255, 0.05)`, color: hexColor, border: `1px solid rgba(255,255,255, 0.1)`, flexShrink:0}}>
...
const MemberDetailScreen=()=>{
  ...
  const card:React.CSSProperties={background:'#0C0C0C',borderRadius:'20px',border:'1px solid rgba(255,255,255,0.08)',padding:'20px'};
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'12px 16px 16px'}}>
        <div style={{...card,display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{position:'relative',marginBottom:'12px',marginTop:'8px'}}>
            ...
            <span style={{position:'absolute',bottom:'1px',right:'1px',width:'14px',height:'14px',borderRadius:'50%',border:'3px solid #0C0C0C',background:sc==='green'?'#34d399':sc==='yellow'?'#fbbf24':'#f43f5e'}}/>
          </div>
        ...
        {showPayModal&&<div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(6px)',zIndex:50,display:'flex',alignItems:'flex-end'}}>
          <div style={{width:'100%',background:'#0C0C0C',borderRadius:'28px 28px 0 0',borderTop:'1px solid rgba(255,255,255,0.1)',padding:'24px 24px 40px',display:'flex',flexDirection:'column',gap:'12px',animation:'slideUp 0.3s ease'}}>
