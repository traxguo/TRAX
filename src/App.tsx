...
import {
  Bell, Home, Users, ChevronLeft, Phone, Mail, Search, Plus, X, Check,
  Clock, BarChart2, Filter, ArrowUpRight, CreditCard, Calendar, UserPlus,
  Dumbbell, CheckCircle2, Zap, MessageCircle, Edit3, Send, Save, Pencil, Trash2,
} from 'lucide-react';
...
export interface Member {
  ...
  endDate: string;
  isActive: boolean;
  pastPayments: { month: string; status: 'paid' | 'partial' | 'pending' }[];
}
...
const mockMembersRaw: Omit<Member,'daysRemaining'>[] = [
  { id:'1', name:'Sarah Jenkins',   phone:'05321234567', email:'sarah@example.com',  img:'', package:'10 Derslik Paket', paymentStatus:'partial', totalAmount:3000,  paidAmount:1500,  startDate:addDays(-62), endDate:addDays(-2),  isActive:false, pastPayments:[{month:'Mart',status:'paid'},{month:'Nisan',status:'partial'}] },
  { id:'2', name:'Marcus Chen',     phone:'05327654321', email:'marcus@example.com', img:'', package:'Aylık Sınırsız',   paymentStatus:'paid',    totalAmount:2000,  paidAmount:2000,  startDate:addDays(-29), endDate:addDays(1),   isActive:true,  pastPayments:[{month:'Nisan',status:'paid'}] },
  { id:'3', name:'Elena Rodriguez', phone:'05339876543', email:'elena@example.com',  img:'', package:'5 Derslik Paket',  paymentStatus:'unpaid',  totalAmount:1500,  paidAmount:0,     startDate:addDays(-1),  endDate:addDays(28), isActive:true,  pastPayments:[] },
  { id:'4', name:'David Kim',       phone:'05301112233', email:'david@example.com',  img:'', package:'12 Derslik Paket', paymentStatus:'partial', totalAmount:3600,  paidAmount:1800,  startDate:addDays(-65), endDate:addDays(-5), isActive:false, pastPayments:[{month:'Mart',status:'paid'},{month:'Nisan',status:'partial'},{month:'Mayıs',status:'pending'}] },
  { id:'5', name:'Ayşe Yılmaz',    phone:'05312223344', email:'ayse@example.com',   img:'', package:'Aylık Sınırsız',   paymentStatus:'paid',    totalAmount:2000,  paidAmount:2000,  startDate:addDays(-10), endDate:addDays(20), isActive:true,  pastPayments:[{month:'Nisan',status:'paid'}] },
  { id:'6', name:'Mehmet Kaya',     phone:'05343334455', email:'',                   img:'', package:'20 Derslik Paket', paymentStatus:'paid',    totalAmount:5000,  paidAmount:5000,  startDate:addDays(-15), endDate:addDays(45), isActive:true,  pastPayments:[{month:'Nisan',status:'paid'}] },
  { id:'7', name:'Fatma Demir',     phone:'05354445566', email:'fatma@example.com',  img:'', package:'Yıllık Üyelik',    paymentStatus:'paid',    totalAmount:18000, paidAmount:18000, startDate:addDays(-180),endDate:addDays(185),isActive:true,  pastPayments:[{month:'Ekim',status:'paid'},{month:'Kasım',status:'paid'}] },
  { id:'8', name:'Can Öztürk',     phone:'05365556677', email:'can@example.com',    img:'', package:'10 Derslik Paket', paymentStatus:'unpaid',  totalAmount:3000,  paidAmount:0,     startDate:addDays(-35), endDate:addDays(-8), isActive:false, pastPayments:[{month:'Mart',status:'pending'}] },
];

export const revenueData = [
  { day:'1', value:18200 },{ day:'2', value:21500 },{ day:'3', value:19800 },
  { day:'4', value:24300 },{ day:'5', value:22100 },{ day:'6', value:28900 },{ day:'7', value:32450 },
];

// ── Colors ────────────────────────────────────────────────────
const A  = '#D97706';
const AL = '#F59E0B';
const AG = 'rgba(217,119,6,0.25)';
...
const GLOBAL_CSS = `
  ...
  @keyframes loginGlow { 0%,100%{opacity:0.6} 50%{opacity:1} }
...
const withDays = (m: Omit<Member,'daysRemaining'>): Member => ({ ...m, daysRemaining: calcDays(m.endDate) });
const initialMembers: Member[] = mockMembersRaw.map(withDays);

const getStatus = (d: number): 'expired'|'warning'|'active' =>
  d < 0 ? 'expired' : d <= 7 ? 'warning' : 'active';

const getStatusLabel = (d: number) =>
  d < 0 ? `${Math.abs(d)} gün önce` : d === 999 ? 'Tarih yok' : d === 0 ? 'Bugün bitiyor' : `${d} gün kaldı`;

const getStatusBadgeLabel = (d: number) => d < 0 ? 'SÜRESİ DOLDU' : `${d} GÜN KALDI`;
const getInitials = (name: string) => name.split(' ').map(n=>n[0]).slice(0,2).join('').toUpperCase();

const avatarBg = (s: 'expired'|'warning'|'active') =>
  s==='expired' ? 'linear-gradient(135deg,#7f1d1d,#991b1b)' :
  s==='warning' ? 'linear-gradient(135deg,#78350f,#92400e)' :
                  'linear-gradient(135deg,#1a3a2a,#14532d)';

const statusBadgeStyle = (s: 'expired'|'warning'|'active'): React.CSSProperties =>
  s==='expired' ? { color:'#fca5a5', background:'rgba(239,68,68,0.12)', border:'1px solid rgba(239,68,68,0.2)' } :
  s==='warning' ? { color:'#fcd34d', background:'rgba(251,191,36,0.1)',  border:'1px solid rgba(251,191,36,0.18)' } :
                  { color:'#86efac', background:'rgba(34,197,94,0.1)',   border:'1px solid rgba(34,197,94,0.15)' };

const cardPulseStyle = (s: 'expired'|'warning'|'active'): React.CSSProperties =>
  s==='expired' ? { animation:'dangerPulse 2.2s ease-in-out infinite', border:'1px solid rgba(239,68,68,0.25)' } :
  s==='warning' ? { animation:'warnPulse 2.5s ease-in-out infinite',   border:'1px solid rgba(251,191,36,0.2)' } :
                  { border:'1px solid rgba(255,255,255,0.06)' };

const dotStyle = (s: 'expired'|'warning'|'active'): React.CSSProperties =>
  s==='expired' ? { background:'#ef4444', animation:'dotPulse 2s ease-in-out infinite' } :
  s==='warning' ? { background:'#fbbf24', animation:'dotPulse 2.5s ease-in-out infinite' } :
                  { background:'#22c55e' };
...
const Login = () => {
  ...
  const handleLogin = () => {
    if (!user || !pass) { setErr('Lütfen tüm alanları doldurun.'); return; }
    setBusy(true); setErr('');
    setTimeout(() => {
      if (user === 'admin' && pass === '1234') { navigate('/app/home'); }
      else { setBusy(false); setErr('Kullanıcı adı veya şifre hatalı.'); }
    }, 700);
  };

  const fi: React.CSSProperties = { width:'100%', padding:'15px 16px', borderRadius:14, fontSize:15, color:'white', outline:'none', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', boxSizing:'border-box' };

  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'0 28px', background:'#0a0a0a', position:'relative', overflow:'hidden' }}>
      <div style={{ position:'absolute', top:'-20%', left:'50%', transform:'translateX(-50%)', width:400, height:400, borderRadius:'50%', background:`radial-gradient(circle,rgba(217,119,6,0.12) 0%,transparent 70%)`, pointerEvents:'none', animation:'loginGlow 4s ease-in-out infinite' }}/>
      <div style={{ width:'100%', maxWidth:360, display:'flex', flexDirection:'column', gap:28, opacity:0, animation:'fadeUp 0.5s ease 0.1s forwards' }}>
        <div style={{ textAlign:'center' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:10 }}>
            <div style={{ width:40, height:40, borderRadius:12, background:`linear-gradient(135deg,${A},${AL})`, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 8px 24px ${AG}` }}>
              <Dumbbell style={{ width:20, height:20, color:'#080808' }}/>
            </div>
            <span style={{ fontSize:28, fontWeight:900, color:'white', letterSpacing:'-0.04em' }}>TRAX</span>
          </div>
          <p style={{ fontSize:13, color:'rgba(255,255,255,0.25)', fontWeight:500 }}>Fitness Üye Takip Sistemi</p>
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <input style={fi} placeholder="Kullanıcı adı" value={user} onChange={e=>setUser(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()}/>
          <input style={fi} placeholder="Şifre" type="password" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()}/>
          {err && <p style={{ fontSize:12, color:'#fca5a5', textAlign:'center' }}>{err}</p>}
          <button onClick={handleLogin} disabled={busy} style={{ width:'100%', padding:16, borderRadius:16, fontWeight:800, fontSize:15, border:'none', cursor:'pointer', background:`linear-gradient(135deg,${A},${AL})`, color:'#080808', boxShadow:`0 8px 24px ${AG}`, opacity:busy?0.7:1, marginTop:4 }}>
            {busy ? 'Giriş yapılıyor...' : 'Giriş Yap'}
          </button>
        </div>
        <p style={{ textAlign:'center', fontSize:11, color:'rgba(255,255,255,0.15)' }}>Demo: admin / 1234</p>
      </div>
    </div>
  );
};
...
const MemberCardSkeleton = () => (
  <div style={{ display:'flex', alignItems:'center', gap:'12px', padding:'14px 16px', borderRadius:'18px', background:'#111', border:'1px solid rgba(255,255,255,0.06)' }}>
    ...
    <div style={{ flex:1 }}><Skeleton w="60%" h={13} mb={6}/><Skeleton w="40%" h={10}/></div>
...
const StatusBadge = ({ days }: { days:number }) => {
  ...
  return <span style={{ fontSize:10, fontWeight:800, padding:'3px 8px', borderRadius:999, letterSpacing:'0.06em', ...statusBadgeStyle(s) }}>{getStatusBadgeLabel(days)}</span>;
};
...
const MemberCard = ({ m, onClick, delay=0 }: { m:Member; onClick:()=>void; delay?:number }) => {
  ...
  return (
    <button className="member-card" onClick={onClick} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'14px 16px', borderRadius:'18px', background:'#111', cursor:'pointer', textAlign:'left', width:'100%', opacity:0, animation:`fadeUp 0.35s ease ${delay}s forwards`, ...(status!=='active' ? cardPulseStyle(status) : { border:'1px solid rgba(255,255,255,0.06)' }) }}>
...
const HeroChart = () => {
  ...
  const visibleData = revenueData.map((d,i) => ({ ...d, value: i < Math.floor(progress/100*revenueData.length) ? d.value : undefined }));
  return (
...
const PaymentBadge = ({ status }: { status:'paid'|'partial'|'unpaid' }) => {
  const cfg = { paid:{color:'rgba(255,255,255,0.5)',label:'Ödeme Tamamlandı'}, partial:{color:'rgba(255,255,255,0.35)',label:'Kısmi Ödeme'}, unpaid:{color:'rgba(255,255,255,0.25)',label:'Ödeme Bekleniyor'} }[status];
      ...
      <div style={{ width:'100%', background:'#111', borderRadius:'28px 28px 0 0', borderTop:'1px solid rgba(255,255,255,0.06)', padding:'28px 24px 48px', display:'flex', flexDirection:'column', gap:20, animation:'slideUp 0.3s ease' }}>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14, textAlign:'center' }}>
          ...
          <div>
            ...
            <p style={{ fontSize:13, color:'rgba(255,255,255,0.35)', lineHeight:1.6 }}><span style={{ color:'rgba(255,255,255,0.7)', fontWeight:600 }}>{name}</span> kalıcı olarak silinecek.</p>
          </div>
        </div>
...
const AddMemberModal = ({ onClose, onAdd }: { onClose:()=>void; onAdd:(m:Member)=>void }) => {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      onAdd({ id:Date.now().toString(), name:form.name, phone:form.phone, email:form.email, img:'', package:form.package, daysRemaining:calcDays(endDisp), paymentStatus:'unpaid', totalAmount:parseInt(form.totalAmount)||0, paidAmount:0, startDate:toDisp(form.startDate)||new Date().toLocaleDateString('tr-TR'), endDate:endDisp, isActive:calcDays(endDisp)>=0, pastPayments:[] });
      onClose();
    }, 500);
  };
  return (
...
const Header = ({ onAddMember, onBell, urgentCount }: { onAddMember?:()=>void; onBell:()=>void; urgentCount:number }) => {
  ...
  return (
    <header style={hdr}>
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        <div style={{ width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${A},${AL})`, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <Dumbbell style={{ width:14, height:14, color:'#080808' }}/>
        </div>
        <span style={{ fontSize:20, fontWeight:900, color:'white', letterSpacing:'-0.04em' }}>TRAX</span>
      </div>
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
...
const BottomNav = () => {
  ...
  return (
    <div style={{ flexShrink:0, height:88, background:'linear-gradient(to top,#0a0a0a 60%,transparent)', display:'flex', alignItems:'flex-end', justifyContent:'center', paddingBottom:20, zIndex:20, animation:'navSlide 0.5s ease 0.1s both' }}>
      <div style={{ background:'rgba(20,20,20,0.95)', backdropFilter:'blur(24px)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:999, height:58, display:'flex', alignItems:'center', gap:0, padding:'0 8px', boxShadow:'0 8px 32px rgba(0,0,0,0.6)' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10, padding:'12px 16px 8px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
        {[{ label:'Süresi Doldu',count:red,icon:'🕐' },{ label:'Bu Hafta',count:yellow,icon:'⏳' },{ label:'Aktif',count:green,icon:'✓' }].map(({ label,count,icon },i) => (
          <div key={label} style={{ background:'#111', border:'1px solid rgba(255,255,255,0.06)', borderRadius:18, padding:'14px 12px', display:'flex', flexDirection:'column', gap:10, opacity:0, animation:`fadeUp 0.35s ease ${0.1+i*0.06}s forwards` }}>
            ...
            <span style={{ fontSize:26, fontWeight:900, color:'white', lineHeight:1 }}>{count}</span>
      ...
      {thisWeek.length > 0 && (
        <div style={{ background:'#111', borderRadius:20, border:'1px solid rgba(255,255,255,0.06)', overflow:'hidden', opacity:0, animation:'fadeUp 0.35s ease 0.32s forwards' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 16px 10px' }}>
            ...
            <Link to="/app/members" style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.25)', textDecoration:'none' }}>Tümü →</Link>
          </div>
      ...
      {thisWeek.length===0 && expired.length===0 && (
        <div style={{ background:'#111', borderRadius:20, border:'1px solid rgba(255,255,255,0.06)', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, padding:'0 12px 12px' }}>
            {members.slice(0,4).map((m,i) => <MemberCard key={m.id} m={m} onClick={()=>navigate(`/app/members/${m.id}`)} delay={i*0.04}/>)}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10, padding:'12px 16px 16px' }}>
      ...
      <div style={{ ...card, display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', opacity:0, animation:'fadeUp 0.35s ease 0.05s forwards' }}>
        ...
        <div style={{ marginBottom:12, marginTop:8 }}><Avatar name={member.name} size={72} status={status}/></div>
        ...
        <div style={{ display:'flex', gap:8, width:'100%' }}>
          ...
          {member.email && <a href={`mailto:${member.email}`} style={{ width:42, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, color:'rgba(255,255,255,0.3)', textDecoration:'none' }}><Mail style={{ width:14, height:14 }}/></a>}
        </div>
      </div>
...
const router = createBrowserRouter([
  { path:'/',       element:<W><style>{GLOBAL_CSS}</style><Login/></W> },
  { path:'/login',  element:<W><style>{GLOBAL_CSS}</style><Login/></W> },
