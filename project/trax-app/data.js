/* ============ TRAX — mock data ============ */
window.TRAX = (function () {
  const avatarColors = ['#c8ff1e','#4be38a','#66a3ff','#ffb84d','#ff6358','#b388ff','#2de2c0','#ff8fab'];
  const colorFor = (name) => {
    let h = 0; for (let i=0;i<name.length;i++) h = (h*31 + name.charCodeAt(i)) % avatarColors.length;
    return avatarColors[h];
  };
  const initials = (name) => name.split(' ').slice(0,2).map(w=>w[0]).join('').toUpperCase();

  // plan: 'Aylık' (tarih bazlı) | 'Paket' (seans bazlı, adet)
  const members = [
  ...
  const stats = {
    active:   { val:'1.284', delta:'+4.2%', up:true,  note:'geçen aya göre' },
    today:    { val:'46',    delta:'+12',   up:true,  note:'dünden fazla' },
    revenue:  { val:'₺184K', delta:'+8.1%', up:true,  note:'bu ay' },
    expiring: { val:'37',    delta:'7 gün içinde', up:false, note:'' },
  };

  return { members, todayCheckins, activity, weekVisits, stats, colorFor, initials, avatarColors };
})();
