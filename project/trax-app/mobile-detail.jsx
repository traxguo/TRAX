/* ============ TRAX mobile — üye detayı (redesign) ============ */
...
function DetailScreen({ id, back, onEdit, onDelete }) {
  const { members, renewMember } = useStore();
  const m = members.find(x => x.id === id);
  const [renewed, setRenewed] = dS(false);
  ...
  const history = [
    ...
    { ico: 'user', t: 'PT seansı', s: '08 May 2026 · 07:00' },
    { ico: 'plus', acc: true, t: 'Üye kaydı oluşturuldu', s: m.joined },
  ];
  const waLink = 'https://wa.me/9' + (m.phone || '').replace(/\D/g, '');

  return (
    <div className="fade detail">
      {/* header — ana sayfa ile aynı ikon stili */}
      ...
      <div className="m-screen" style={{ paddingTop: 4 }}>
        {/* hero banner */}
        <div className="dhero" style={{ '--sc': c, '--srgb': rgb }}>
          <div className="dhero-ring">
            <div className="dhero-av" style={{ background: D.colorFor(m.name) }}>{D.initials(m.name)}</div>
          </div>
          ...
          <div className="dhero-pills">
            <span className="dpill" style={{ color: c, background: 'rgba(' + rgb + ',0.14)', borderColor: 'rgba(' + rgb + ',0.3)' }}>
              <span className="dd" style={{ background: c }}></span>{statusLabel(g)}
            </span>
        ...
        {/* membership progress / paket durumu */}
        {m.kind === 'paket' ? (
          <div className="card dcard" style={{ marginTop: 16 }}>
            ...
            <div className="bar" style={{ height: 9 }}><i style={{ width: Math.max(6, Math.min(100, m.adet * 10)) + '%', background: c, boxShadow: '0 0 12px ' + c }}></i></div>
          ...
          <div className="card dcard" style={{ marginTop: 16 }}>
            ...
            <div className="bar" style={{ height: 9 }}><i style={{ width: pct + '%', background: c, boxShadow: '0 0 12px ' + c }}></i></div>
            <div className="row" style={{ marginTop: 9, justifyContent: 'space-between' }}>
              <span className="muted" style={{ fontSize: 11.5 }}>Başlangıç · {m.joined}</span>
        ...
        <div className="card" style={{ padding: '6px 16px' }}>
          ...
          <a className="crow" href={waLink} target="_blank" rel="noreferrer">
            <div className="crow-ic" style={{ color: 'var(--ok)', background: 'rgba(70,224,138,0.12)', borderColor: 'rgba(70,224,138,0.25)' }}><Icon name="whatsapp" size={16} /></div>
        ...
        {renewed ? (
          <div className="row" style={{ justifyContent: 'center', gap: 9, color: 'var(--ok)', fontWeight: 650, padding: '20px 0 6px' }}>
            <Icon name="check" size={18} stroke={2.4} />{m.kind === 'paket' ? '10 seans eklendi' : 'Üyelik yenilendi'}
          </div>
        ) : (
          <button className="btn primary" style={{ marginTop: 18 }} onClick={() => { renewMember(m.id); setRenewed(true); }}>
            <Icon name="trend" size={17} />{m.kind === 'paket' ? 'Pakete 10 seans ekle' : 'Üyeliği yenile'}
          </button>
        )}
        <button className="btn danger" style={{ marginTop: 10 }} onClick={() => onDelete(m)}>
